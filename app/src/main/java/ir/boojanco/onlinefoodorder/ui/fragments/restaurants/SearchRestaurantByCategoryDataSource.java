...
public class SearchRestaurantByCategoryDataSource extends PageKeyedDataSource<Integer, RestaurantList> {
    private static final String TAG = SearchRestaurantByCategoryDataSource.class.getSimpleName();
    ...
    public SearchRestaurantByCategoryDataSource(RestaurantRepository restaurantRepository, RestaurantDataSourceInterface dataSourceInterface, String categoryName, String city) {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, RestaurantList> callback) {
        ...
        Observable<RestaurantResponse> observable = restaurantRepository.searchByCategoryObservable(categoryName, city, FIRST_PAGE, PAGE_SIZE);
        if (observable != null) {
    ...
    public void loadBefore(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, RestaurantList> callback) {
        ...
        Observable<RestaurantResponse> observable = restaurantRepository.searchByCategoryObservable(categoryName, city, params.key, PAGE_SIZE);
        if (observable != null) {
            observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<RestaurantResponse>() {
                @Override
                public void onCompleted() {
